...
| `entity_query` | array/object | an entity query — type/bundle, two sort levels, reference & taxonomy-hierarchy filters, **shared-reference matching** ("shares a category and/or an author with this page", ranked best-match-first), exclude-the-current-entity, offset/length, optional pager |
...
Rules of thumb:
- **Which element the root is, is a content decision — nothing in the styling system keys off the tag.** The examples above all show `<div>` because it is the neutral choice, but use `<section>` when the band is a real section of the page with its own heading, `<article>` for a standalone document (a job posting, a person, a project), `<header>` / `<footer>` for the page furniture. Style props, seam collapsing, the region-flush utilities and the editor chrome all match classes, never element names, so the semantics are yours to pick.
...
- **Nesting inside another component's region: nothing to do in the child.** A parent that renders a `region` strips its children's side gutters with `neo-region-flush-x`, and reclaims their outer vertical spacing with `neo-region-flush-y` / `-t` / `-b` — see [insight_body](web/themes/front/components/insight_body/insight_body.twig), whose article region hosts Text, Accordion and Testimonial. `-x` zeroes `--spacing-container` on the child's content wrapper and matches **both** spellings, `container-content` or its longhand `container-center px-container`, so write the wrapper whichever way reads best; the vertical three zero the `--spacing-component-t` / `-b` channel on the region's first and/or last child, whatever element that child's root happens to be, and the channel inherits down to the `neo-section-y` carrier even in the deep-carrier layout. Never hand-roll a "flush" variant in the child, and don't add a prop for it: which spacing survives is the *parent's* decision, made once on the region.
