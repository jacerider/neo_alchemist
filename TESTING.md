...
| `Kernel/EntityQueryValueTest` | The query provider's list cache tags — attached even for an empty result set, so the first matching entity invalidates the listing — plus its block-mode default, the query exposed as a prop-shape context, and the shared-reference filter: OR/AND set algebra, an empty host field degrading to unfiltered rather than to nothing, **the range counting distinct entities and not matched field deltas**, breadth-over-depth ranking with the sort as tie-break, exclude-self, and the host's cache tag reaching the component |
...
| `Kernel/MoveAndLibraryPositionUrlTest` | The `move` and library-position rels gain server-side URL generation in every scope: `move` carrying its direction (and optional parent query) as a generated route, `library` carrying a before/after position and parent as query, both through `Url::fromRoute()`/`Entity::toUrl()` in the entity, field-UI and block scopes — asserted under a non-standard base path (the case client string-concatenation gets wrong), plus the block scope still refusing purge and a directionless move failing loudly |
| `Unit/EditorOpInventoryTest` | The editor's op vocabulary as one table: the eight ops in render order, and for each its verb, position/direction, target route rel, label and icon — the strings the chrome renders today, pinned once so a fourth silent derivation cannot drift |
| `Kernel/EditorOpInventoryUrlTest` | Every op resolves to a URL in every host scope: each op's declared rel is a route the family registers in the entity, field-UI and block scopes (an op naming a rel no scope serves fails here), and each op resolves through the entity-scope generators reading the rel/position/direction off the record rather than re-parsing the identifier |
| `Kernel/ComponentEmitsOpRecordsTest` | The component emits each editor op as a record — id, permitted, url, label, verb, position — for a real instance in the entity, field-UI and block scopes: the eight ops present with the right fields, each carrying the URL the route family spells for its rel (add ops the sibling query, move ops the direction), `permitted` mirroring the same access() decision the boolean did, move up/down withheld at the boundary and on an unknown position, and the records reaching the unchanged data-component attribute end to end |
| `Kernel/EditorChromeIteratesInventoryTest` | The editor chrome projects the static op inventory rather than naming ops: `_neo_alchemist_overlay_ops()` places the four verb ops in the bottom bar's left group, the four positional ops in its right group and split across the two outline edge clusters (before/up on top, after/down on bottom) by each op's own fields, with the labels/icons/placement the chrome renders today — and appending a synthetic row to the inventory adds a button with no preprocess or template edit (the ticket-09 claim, demonstrated); reads no per-component record, so it is green independent of the payload work |
