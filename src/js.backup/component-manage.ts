(function (Drupal, once, displace) {
        ...
        case 'focus':
          if (iframe instanceof HTMLIFrameElement && iframe.contentWindow) {
            if (iframe.contentWindow.scrollX === 0 && iframe.contentWindow.scrollY === 0) {
              let clientWidth = iframe.clientWidth;
              const clientHeight = iframe.clientHeight;
              const sidebar = document.querySelector('.neo-alchemist--form-side');
              if (sidebar) {
                clientWidth -= sidebar.clientWidth;
              }
              const left = (data.width - clientWidth) / 2;
              const top = (data.height - clientHeight) / 2;
              iframe.contentWindow.scrollTo({top: top, left: left, behavior: 'instant' as any});
            }
          }
          break;
        case 'drag_start':
          if (iframe instanceof HTMLIFrameElement && iframe.contentWindow) {
            baseFrameX = iframe.contentWindow.scrollX;
            baseFrameY = iframe.contentWindow.scrollY;
          }
          break;
        case 'drag_move':
          if (iframe instanceof HTMLIFrameElement && iframe.contentWindow) {
            iframe.contentWindow.scrollTo({top: baseFrameY + data.offsetY, left: baseFrameX + data.offsetX, behavior: 'instant' as any});
          }
          break;
      }
    }
  ...
  Drupal.behaviors.neoAlchemistComponentManage = {
    attach: function () {
      ...
      once('neo.alchemist', '#neo-alchemist--iframe, #neo-alchemist--iframe-form').forEach(el => {
        ...
        el.onload = function() {
          if (el instanceof HTMLIFrameElement && el.contentWindow) {
            ...
            iframeDocument.addEventListener('mousedown', function(_event) {
              // Store active iframe so we can support multiple instances on
              // the same page.
              iframe = el;
            });
          }
      ...
      [
        ...
        once('neo.alchemist', '.neo-alchemist-manage--size-' + data.id).forEach(el => {
