...
final class FieldMatcher {
  ...
  public function getMatches(ComponentShapePluginInterface $shape): array {
    ...
    $matches = $this->matchEntityProps($entity_data_definition, 1, $shape->getSchemaType(), $shape->isRequired(), $shape->getSchema());
  ...
  private function matchEntityPropsForIterable(EntityDataDefinitionInterface $entity_data_definition, int $levels_to_recurse, SdcPropJsonSchemaType $primitive_type, bool $is_required_in_json_schema, ?array $schema): array {
    ...
    $all_object_props = [];
    $object_prop_matches = [];
    foreach ($this->iterateJsonSchema($schema) as $name => [
